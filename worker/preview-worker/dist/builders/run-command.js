...
export async function runCommand(cmd, args, cwd, timeoutMs, env) {
    const meta = {
        command: cmd,
        args: [...args],
...
export async function npmInstall(cwd, pm, opts) {
    const ignoreScripts = config.allowNpmScripts ? [] : ["--ignore-scripts"];
    const env = previewInstallEnv();
    if (pm === "pnpm") {
        return runCommand("pnpm", ["install", "--frozen-lockfile", ...ignoreScripts], cwd, config.installTimeoutMs, env);
    }
    if (pm === "yarn") {
...
export async function npmRunBuild(cwd, pm, script = "build") {
...
async function resolveViteCli(cwd) {
...
export async function runViteBuild(cwd, pm, script = "build") {
    const env = previewBuildEnv();
