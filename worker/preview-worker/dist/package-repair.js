...
export function repairPackageJsonContent(raw, input) {
    const repairs = [];
    let parsed;
    try {
    ...
    const needsVite = buildUsesVite || input.isBase44 || Boolean(allDeps["@vitejs/plugin-react"]);
...
export async function applyPackageRepair(layout, files, isBase44) {
    ...
    try {
        ...
        summary = {
            ...repaired.summary,
            packageManager: pm,
            buildUsesVite: scriptUsesVite(JSON.parse(repaired.content).scripts ?? {}),
            base44ShimReady: isBase44,
            packageJsonRelative,
            projectRoot,
        };
        ...
        summary.repairs.push(e instanceof Error ? e.message : "package.json missing — skipped repair");
        summary.summary = summary.repairs.join("; ");
...
export async function listNodeModulesBin(projectRoot) {
...
export async function viteBinaryExists(projectRoot) {
...
export async function assertViteBinaryPresent(projectRoot, diagnostics) {
