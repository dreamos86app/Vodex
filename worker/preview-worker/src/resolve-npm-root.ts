...
export function packageJsonCandidates(files: WorkspaceFile[]): string[] {
  return files
    .map((f) => norm(f.path))
...
export async function resolveNpmProjectLayout(
  workspaceRoot: string,
  files: WorkspaceFile[],
): Promise<NpmProjectLayout | null> {
