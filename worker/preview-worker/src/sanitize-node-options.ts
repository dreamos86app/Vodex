...
const UNSAFE_NODE_OPTIONS =
  /\$|harmony-import|no-harmony|experimental-wasm|experimental-wasm-imported-strings|experimental-wasm-memory64|experimental-wasm-exnref/i;

/**
 * Vite/npm child builds only — never set on the worker process itself.
 */
export function childBuildNodeOptions(maxOldSpaceMb: number): string {
 ...
 */
export function sanitizeInheritedNodeOptionsForWorkerBoot(): void {
