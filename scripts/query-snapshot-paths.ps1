...
$pub = Invoke-RestMethod -Uri "$base/published_apps?project_id=eq.$projectId&select=snapshot_files" -Headers $h | Select-Object -First 1
$paths = @($pub.snapshot_files | ForEach-Object { $_.path })
@{
  total = $paths.Count
  hasPackage = ($paths -match 'package\.json').Count -gt 0
  hasIndex = ($paths -match 'index\.html').Count -gt 0
  pageFiles = @($paths | Where-Object { $_ -match 'page\.(tsx|jsx)$|/pages/' } | Select-Object -First 8)
} | ConvertTo-Json
